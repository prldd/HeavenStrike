...
func _run() -> void:
	...
	assert(game.board.attack_frame_animation_enabled)
	...
	for faction in game.MechaConceptCatalogScript.FACTIONS:
		var body_image: Image = game.unit_creator_view._body_textures[faction].get_image()
		var weapon_image: Image = game.unit_creator_view._weapon_textures[faction].get_image()
		assert(body_image.get_size() == Vector2i(1254, 1254))
		assert(weapon_image.get_size() == Vector2i(1254, 1254))
		assert(body_image.get_pixel(0, 0).a < 0.05)
	...
	assert(game.unit_creator_pose_option.item_count == 2)
	...
	game.unit_creator_pose_option.select(1)
	game.unit_creator_pose_option.item_selected.emit(1)
	assert(game.unit_creator_view.recipe.pose == "Fire Cycle")
	...
	assert(is_equal_approx(game._animation_duration(1.0), 2.0))
	assert(game.board._attack_frame_duration(0.48) / Engine.time_scale >= 0.30)
	game._cycle_resolution_speed()
