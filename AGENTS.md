...
| `lore_card_store.gd` | Seen-state for the campaign prologue and per-chapter intro cards, stored in the `lore` section of `user://player.cfg`. |
...
- **Edit campaign narrative text:** change `MISSION_STORIES`/`ENCOUNTER_RULES`/`CAMPAIGN_PROLOGUE`/`CHAPTER_CARDS` in `scripts/story_quest_catalog.gd` or `INTERLUDES`/`CHARACTERS` in `scripts/story_dialogue_catalog.gd`, then regenerate the consolidated reading copy `documentation/Campaign_Narrative_Script.md` with `./tools/godot-headless.sh --script res://tools/generate_campaign_narrative.gd`. Never edit the digest directly; it is generated. Voice, structure, and canon rules for all campaign copy live in `documentation/Narrative_Style_Guide.md` — follow it when writing or revising text. The prologue and chapter cards surface in-game through the lore-card overlay in `main.gd` (shown once via `lore_card_store.gd` seen-state, replayable with the WORLD BRIEF button on the operations map); chapters without a `CHAPTER_CARDS` entry simply show no card.
