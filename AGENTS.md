...
| `mission_rules.gd` | Normalizes authored encounter objectives and modifiers, formats mission intel, validates blocked/setup/reinforcement data, and evaluates deterministic win/loss conditions. Seven objective types: `defeat_conductor` (default), `survive`, `eliminate_target`, `protect`, `rout` (kill quota; the enemy Conductor cannot be attacked), `preserve` (lose if a `preserved`-role enemy unit dies or player losses exceed `max_losses`), and `resonance` (win by spending `amount` total mana; Conductor kill remains the alternate win). |
...
| `lore_card_store.gd` | Seen-state for the campaign prologue, per-chapter intro cards, and pre-mission preludes, stored in the `lore` section of `user://player.cfg`. |
| `story_quest_catalog.gd` | Builds the 77-mission original campaign from authored reward pools, enemy decks, and Conductor configurations. `MISSION_STORIES` holds per-mission story text (chapter label, briefing, debriefing) keyed by 1-based mission number. Also owns `CAMPAIGN_PROLOGUE` (first-open orientation), `CHAPTER_CARDS` (per-chapter intro briefings keyed by chapter name), `chapter_number`, and `CAMPAIGN_EPILOGUE`. |
| `story_dialogue_catalog.gd` | Editable campaign dialogue scenes. `INTERLUDES` (post-mission) and `PRELUDES` (pre-mission, shown once when the mission is the campaign frontier) are keyed by 1-based mission number and hold a scene title, location, and ordered speaker/text lines; `CHARACTERS` owns speaker roles, initials, and accent colors. `main.gd` owns only the dialogue presentation and return flow. |
...
- **Authored encounter rules:** optional rules live in `StoryQuestCatalog.ENCOUNTER_RULES`, keyed as `"<1-based mission>:<0-based encounter>"`, and are normalized by `MissionRules`. Encounters without an entry must retain the standard defeat-the-Conductor behavior. Setup and reinforcement deployments must be recorded with their exact cell and mission-role metadata so version-one replays can reconstruct them without executing live rule setup. Rout/preserve/resonance quotas depend on battle counters (`mission_kills`/`mission_losses`/`mission_mana_spent` in `main.gd`, reset per encounter) that `main.gd` passes to `MissionRules.evaluate`/`objective_banner` via the trailing `context` dict — every call site must pass it, including the inline evaluate in `_resolve_side`. The `allow_fallback` rule flag (implied by `rout`) enables the FALL BACK command-dock button: a selected living player unit on the far column (`COLS - 2`) with no mission role returns to hand as a freshly rebuilt card (catalog stats, never battle-buffed runtime stats), freeing its locked mana via `available_mana`; the recall is recorded as a `unit_recalled` replay event (handled in `_apply_next_replay_event`).
...
- `assets/units/attack/<art id>.png` — optional per-unit 3×2 attack atlases extracted from generated video clips. Each atlas contains six 512×512 cells, ordered left-to-right across two rows. When an imported atlas exists for a unit's art id, `BoardView` selects its cells during unit and Conductor attacks (progress driven by `unit_attack_frame_progress`) and skips the overlapping generic class effect; units without one keep the static-sprite lunge and class effect. Generated frames use a minimum wall-clock playback duration so all six remain visible at 2×/4× speed and during autobattle whenever `ANIM ON`; `ANIM OFF` disables them explicitly. Cells preserve the idle sprite's canvas proportions and foot line. Generate them with the Python pipeline:
  ...
  ```
  Install `tools/attack_anim/requirements.txt` into the gitignored `.tools/anim-pipeline-venv`; `ffmpeg` must be on PATH. The script requires exactly six timestamps (override with `--times` in seconds), crops letterbox bars, keys out the white background and gray ground shadow, and rescales/re-anchors every cell onto the idle sprite's canvas — the scale comes from the last (settled) frame, feet aligned to the idle sprite's foot line. Review images go to the ignored `.tools/attack-previews/` directory. Open Godot after extraction so atlases are imported for export-safe runtime loading. `tests/attack_animation_visual_check.gd` directly exercises art ID 241 without depending on player save data.
...
- **Edit campaign narrative text:** change `MISSION_STORIES`/`ENCOUNTER_RULES`/`CAMPAIGN_PROLOGUE`/`CHAPTER_CARDS` in `scripts/story_quest_catalog.gd` or `PRELUDES`/`INTERLUDES`/`CHARACTERS` in `scripts/story_dialogue_catalog.gd`, then regenerate the consolidated reading copy `documentation/Campaign_Narrative_Script.md` with `./tools/godot-headless.sh --script res://tools/generate_campaign_narrative.gd`. Never edit the digest directly; it is generated. Voice, structure, and canon rules for all campaign copy live in `documentation/Narrative_Style_Guide.md` — follow it when writing or revising text. The prologue and chapter cards surface in-game through the lore-card overlay in `main.gd` (shown once via `lore_card_store.gd` seen-state, replayable with the WORLD BRIEF button on the operations map); chapters without a `CHAPTER_CARDS` entry simply show no card. A `PRELUDES` scene plays once after the lore cards when its mission is the uncompleted campaign frontier (`main.gd:_maybe_show_lore_cards` chains it via `pending_prelude_mission`), and the mission-select SCENE button replays it until the mission is completed.
